    ...
    last_report.each {|r| @time = r[1]}
        ...
        unless @diff.to_i < 7200
            ...
            hosts.each do |h|
              ...
              if Ping.pingecho("#{h[2]}", 10) == true
                puts "#{h[2]} ping sucessful"
              else
                pings_down = pings_down + 1
                broke_sites = broke_sites + "#{h[2]},\n"
                body = body + h[1] + " unable to ping, go take a look at this host."
              end
              ...
              sites.each do |s|
                ...
                puts s.inspect
                  ...
                  unless c.response_code < 399
                      ...
                      sites_down = sites_down + 1
                      broke_sites = broke_sites + "#{s[2]},\n"
                      body = body + s[1] + " unable to curl, check this site. "
                          puts "--DOWN--"
                  end 
              end
            end
